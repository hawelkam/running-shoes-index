...
async function get2026Stats(): Promise<StatsData> {
  const query = `*[
    _type == "runningShoe" &&
    !defined(nextVersion) &&
    defined(releaseInfo.pl.date) &&
    releaseInfo.pl.date >= "2026-01-01" &&
    releaseInfo.pl.date <= "2026-12-31"
  ]{
    _id,
    name,
    slug,
  ...
  try {
    ...
    console.error("Failed to fetch 2026 stats:", error);
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <header className="mb-8">
        ...
        <p className="text-lg text-gray-600 mb-4">
          Statistics for running shoes released in Poland throughout 2026
        </p>
        <p className="text-sm text-gray-500">
          Data period: January 1, 2026 - December 31, 2026
        </p>
      </header>

      {/* Full Year Statistics Cards */}
      <div className="space-y-8">
        <StatCard title="All Shoes Released in 2026" stats={allShoes} />
        <StatCard
          title="Daily Trainers Released in 2026"
          stats={dailyTrainers}
        />
      </div>
      ...
      {rawShoes.length === 0 && (
        <div className="mt-8 bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
          ...
          <p className="text-yellow-700">
            Statistics for 2026 will populate as shoes are released throughout
            the year.
          </p>
        </div>
      )}
